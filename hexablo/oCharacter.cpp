...
void oCharacter::Update(float dt) {
    ...
    if (jleCore::core->input->keyboard->GetKeyPressed('T')) {
        SetHexagonPlacementInterp(hexHelperFunctions::GetRandInt(0, 10),
                                  hexHelperFunctions::GetRandInt(0, 10));
    }
    const auto lerpVec2 =
    ...
    if (mInterpingPosition) {
        ...
        auto pos = lerpVec2({mHexagonPixelX, mHexagonPixelY},
                            {mInterpingX, mInterpingY},
                            mInterpingAlpha);
...
void oCharacter::SetHexagonPlacementTeleport(int q, int r) {
    ...
    auto p = hexHexagonFunctions::HexToPixel(
        q, r, world->mHexSizeX, world->mHexSizeY);
...
void oCharacter::FromJson(const nlohmann::json& j_in) {
    ...
    JLE_FROM_JSON_WITH_DEFAULT(
        j_in, mInterpBetweenHexasSpeed, "mInterpBetweenHexasSpeed", 1.f);

    JLE_FROM_JSON_WITH_DEFAULT(j_in,
                               mAttackCooldownAfterAnimationMs,
                               "mAttackCooldownAfterAnimationMs",
                               0.f);

    JLE_FROM_JSON_WITH_DEFAULT(
        j_in, mHealthBarObjectTemplatePath, "mHealthBarObjectTemplatePath", "");
    ...
    JLE_FROM_JSON_WITH_DEFAULT(
        j_in, mNorthwestTextureX, "mNorthwestTextureX", 32);
    JLE_FROM_JSON_WITH_DEFAULT(
        j_in, mNorthwestTextureY, "mNorthwestTextureY", 0);
    ...
    JLE_FROM_JSON_WITH_DEFAULT(
        j_in, mNortheastTextureX, "mNortheastTextureX", 96);
    JLE_FROM_JSON_WITH_DEFAULT(
        j_in, mNortheastTextureY, "mNortheastTextureY", 0);
    ...
    JLE_FROM_JSON_WITH_DEFAULT(
        j_in, mSoutheastTextureX, "mSoutheastTextureX", 160);
    JLE_FROM_JSON_WITH_DEFAULT(
        j_in, mSoutheastTextureY, "mSoutheastTextureY", 0);
    ...
    JLE_FROM_JSON_WITH_DEFAULT(
        j_in, mSouthwestTextureX, "mSouthwestTextureX", 224);
    JLE_FROM_JSON_WITH_DEFAULT(
        j_in, mSouthwestTextureY, "mSouthwestTextureY", 0);
}
...
void oCharacter::Attack(oCharacter::oCharacterDirection dir) {
    ...
    const double animationTimeMs =
        mAseprite->GetActiveAsepriteRef().GetTotalAnimationTimeMs();
    ...
    jleCore::core->GetTimerManager().ExecuteFuncInSecondsWeakData(
        animationTimeMs * 0.001, futureFunc, GetWeakPtrToThis());

    // Can not attack again for the animation time + some additional time
    jleCore::core->GetTimerManager().ExecuteFuncInSecondsWeakData(
        (animationTimeMs + mAttackCooldownAfterAnimationMs) * 0.001,
...
void oCharacter::LookAtPosition(int x, int y) {
    ...
    glm::vec2 target = {x, y};
    glm::vec2 origin = {mTransform->GetWorldX() + smallXAdjustment,
