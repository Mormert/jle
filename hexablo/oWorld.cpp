...
void oWorld::Start() {
    ...
    _quadRenderingPtr = jleCore::core->rendering->quads.get();
...
void oWorld::Update(float dt) {
  ...
  //  RenderVisualWorld(dt);
    ...
    /*for (int i = 0; i < 25; i++) {
        for (int j = 0; j < 25; j++) {
            const auto realCoords = hexHexagonFunctions::HexToPixel(i, j, _hexSizeX, _hexSizeY);
    ...
    const auto hexCoord = hexHexagonFunctions::PixelToHex(mx, my, _hexSizeX, _hexSizeY);

    quad.textureX = _hexagonTiles[0]._textureX;
    quad.depth = 0.5f;
    const auto realCoords = hexHexagonFunctions::HexToPixel(hexCoord.x, hexCoord.y, _hexSizeX, _hexSizeY);
    quad.x = realCoords.x - hexagonTile._width / 2;
    quad.y = realCoords.y - hexagonTile._height / 2;
    _quadRenderingPtr->SendTexturedQuad(quad, RenderingMethod::Dynamic);*/
...
void oWorld::FromJson(const nlohmann::json &j_in) {
    JLE_FROM_JSON_WITH_DEFAULT(j_in, _worldHexTilesAsepritePath, "_worldHexTilesAsepritePath", "");
    ...
    if (tiles != j_in.end()) {
        _hexagonTiles = j_in.at("_hexagonTiles").get<std::vector<HexagonTile>>();
    }
    if (_aseprite._frames.empty()) {
        LoadTilesTexture();
    }
...
void oWorld::LoadTilesTexture() {
    if (!_worldHexTilesAsepritePath.empty()) {
        _aseprite.LoadFromFile(jleRelativePath{_worldHexTilesAsepritePath}.GetAbsolutePathStr());
        //_worldHexTilesTexture = jleCore::core->texture_creator->LoadTextureFromPath(truePath);
    }
}
...
bool oWorld::IsHexagonWalkable(int q, int r) {
    ...
    if (_staticallyNotWalkableHexagons.find({q, r}) != _staticallyNotWalkableHexagons.end()) {
        return false;
    }
...
void oWorld::RenderVisualWorld(float dt) {
    ...
    for (int i = 0; i < _worldHexagons.size(); i++) {
        for (int j = 0; j < _worldHexagons[i].size(); j++) {
            ...
            TexturedQuad quad{_aseprite._imageTexture, frame._frame._x + hexagonTile._textureX, frame._frame._y+ hexagonTile._textureY,
