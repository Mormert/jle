...
glm::ivec2 hexHexagonFunctions::HexToPixel(int hexQ, int hexR, int hexSizeX, int hexSizeY) {
...
// The way we get the integer hex position is by doing the inverse of the HexToPixel function,
// but since we will end up with a fractal hexagonal coordinate, we round the coordinates and get
// 4 candidates, which we try again using the distance function. We also need to account for the
// row multiplier, so we are testing against 8 different points.
glm::ivec2 hexHexagonFunctions::PixelToHex(int pixelX, int pixelY, int hexSizeX, int hexSizeY) {
    ...
    const auto f = [&](float oddRowMultiplier) -> void {
        ...
        nearPixelPlaces.emplace_back(HexToPixel(hexQ_upper, hexR_upper, hexSizeX, hexSizeY),
                                     glm::ivec2{hexQ_upper, hexR_upper});
        nearPixelPlaces.emplace_back(HexToPixel(hexQ_upper, hexR_lower, hexSizeX, hexSizeY),
                                     glm::ivec2{hexQ_upper, hexR_lower});
        nearPixelPlaces.emplace_back(HexToPixel(hexQ_lower, hexR_upper, hexSizeX, hexSizeY),
                                     glm::ivec2{hexQ_lower, hexR_upper});
        nearPixelPlaces.emplace_back(HexToPixel(hexQ_lower, hexR_lower, hexSizeX, hexSizeY),
    ...
    glm::ivec2 hexPlace = nearPixelPlaces[0].second;    // assign at least something
    for (auto place: nearPixelPlaces) {
        auto newDistance = glm::distance(glm::vec2{(float) pixelX, (float) pixelY}, glm::vec2{place.first});
