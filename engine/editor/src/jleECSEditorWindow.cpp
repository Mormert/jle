...
class SetParentCommand : public jleUndoRedoCommandBase
{
public:
    SetParentCommand(const jlECS::ObjectRef& child,
                     const std::optional<jlECS::ObjectRef>& parent)
        : _child(child)
        , _parent(parent)
    {
    }
    ...
    void execute(const CommandContext& ctx) override
    {
        ...
        {
            _oldParent = childParentComponent->getParentRef(_child.getECS());
        }

        // If there's a new parent, store its current parent
        if (_parent.has_value())
        {
            if (auto* newParentComponent = _parent->getComponentPtr<cParent>())
            {
                _oldParentOfNewParent = newParentComponent->getParentRef(_parent->getECS());
            }
        }
...
private:
    jlECS::ObjectRef _child;
    std::optional<jlECS::ObjectRef> _parent{};

    // Child's original parent
    std::optional<jlECS::ObjectRef> _oldParent{};

    // New parent's original parent (to handle the case where the new parent was modified too)
...
jleECSEditorWindow::renderUI(const RenderUIInput& input)
{
    ...
    handleUndoRedoShortcuts(input, undoRedoCommandCtx);
    ...
    handleSaveLoadButtons(input, undoRedoCommandCtx, ecs, serializationContext);
    ImGui::SameLine();
    handleObjectHierarchy(input, undoRedoCommandCtx, ecs, serializationContext);
    ImGui::SameLine();
    handleSelectedObjectsPane(input, undoRedoCommandCtx, ecs);

    handleDeletionConfirmation(input, undoRedoCommandCtx, ecs);
        ...
        if (_selectedObjects->size() == 1)
        {
            ...
            if (selectedObject.isValid())
            {
                ...
                for (auto &comp : components)
                {
                    ...
                    std::string serializedBinaryDataBefore = getSerializedBinaryStringFromComponent(*comp, input.editorUpdate.engineUpdateContext.serializationContext);
                    jleImGuiArchive imGuiArchive{input.editorUpdate};
                    comp->imGuiSerialize(imGuiArchive, selectedObject.objectIndex());
                    std::string serializedBinaryDataAfter = getSerializedBinaryStringFromComponent(*comp, input.editorUpdate.engineUpdateContext.serializationContext);
