...
namespace jle {
    ...
    inline glm::vec2 Vec2(const ImVec2 vec) {
        return {vec.x, vec.y};
    }
    ...
    private:
        void RebuildTaskStats(size_t endFrame, size_t framesCount) {
            ...
            std::sort(statPriorities.begin(), statPriorities.end(),
                      [this](size_t left, size_t right) { return taskStats[left].maxTime > taskStats[right].maxTime; });
        ...
        void RenderGraph(ImDrawList *drawList, glm::vec2 graphPos, glm::vec2 graphSize, size_t frameIndexOffset) {
            ...
            for (size_t frameNumber = 0; frameNumber < frames.size(); frameNumber++) {
                ...
                glm::vec2 framePos = graphPos +
                                     glm::vec2(graphSize.x - 1 - frameWidth - (frameWidth + frameSpacing) * frameNumber,
                                               graphSize.y - 1);
        ...
        void RenderLegend(ImDrawList *drawList, glm::vec2 legendPos, glm::vec2 legendSize, size_t frameIndexOffset) {
            ...
            for (size_t taskIndex = 0; taskIndex < currFrame.tasks.size(); taskIndex++) {
                ...
                uint32_t textColor = useColoredLegendText ? task.color : RGBA_LE(0xF2F5FAFFu);
        ...
        static void
        Rect(ImDrawList *drawList, glm::vec2 minPoint, glm::vec2 maxPoint, uint32_t col, bool filled = true) {
            if (filled)
        ...
        static void
        RenderTaskMarker(ImDrawList *drawList, glm::vec2 leftMinPoint, glm::vec2 leftMaxPoint, glm::vec2 rightMinPoint,
    ...
    public:
        ...
        void Render() {
            ...
            title.precision(2);
            title << std::fixed << "Profiler [" << 1.0f / avgFrameTime << " FPS\t" << avgFrameTime * 1000.0f
                  << "ms]###ProfilerWindow";
        ...
        bool stopProfiling;
        int frameOffset;
        jle::ProfilerGraph cpuGraph;
        jle::ProfilerGraph gpuGraph;
        int frameWidth;
        int frameSpacing;
        bool useColoredLegendText;
