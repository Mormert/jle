...
void jleQuadRendering_OpenGL::SendTexturedQuad(TexturedQuad& texturedQuad,
...
void jleQuadRendering_OpenGL::ProcessTexturedQuads(
    ...
    std::unordered_map<std::shared_ptr<jleTextureInterface>, std::vector<QuadData>>
        quadDataMap;
    ...
    for (auto&& key : quadDataMap) {
        ...
        if (!key.first->IsActive()) {
            ...
            quadShaderInstanced.SetVec2(
                "textureDims",
                glm::vec2{float(key.first->GetWidth()),
                          float(key.first->GetHeight())});
