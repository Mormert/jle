...
// TODO: Update connected variable on this event instead of directly in
// Connect()
EMSCRIPTEN_KEEPALIVE
...
EMSCRIPTEN_KEEPALIVE
void on_socket_disconnect() { sNetworkingJs_onClosed(0); }

EMSCRIPTEN_KEEPALIVE
void on_socket_failed() { sNetworkingJs_onFailed(); }
}
...
void jleNetworking::Connect(
    ...
    std::string uri_https =
        std::regex_replace(uri, std::regex("http"), "https");

    // Note that we set the socket on the window object, such that it will be
    // globally accessible.
    ...
    EM_ASM(window.socket.on(
        'msgpack',
        data = >
               {
                   stringData = data;
                   var ptr =
                       allocate(intArrayFromString(stringData), ALLOC_NORMAL);
               ...
               () = >
                    {
                        ...
                        var ptr = allocate(intArrayFromString(stringData),
                                           ALLOC_NORMAL);
           ...
           window.socket.on(
               'disconnect', () = > { _on_socket_disconnect(); });

           window.socket.on(
               'connect_error', data = > { _on_socket_failed(); });
...
void jleNetworking::Disconnect() {
    ...
    EM_ASM(window.socket.disconnect(););
...
void jleNetworking::EmitJsonData(const std::string& event,
    ...
    std::string js_eval_string =
        "window.socket.emit(\"msgpack\", " + to_string(json_package) + ");";
...
void jleNetworking::EmitEvent(const std::string& event,
    ...
    // We must replace all \n with \\n for it to not fail eval execution in
    // JavaScript.
    js_eval_string =
        std::regex_replace(js_eval_string, std::regex("\n"), "\\n");
