...
void jle::SceneEditorWindow::Update(jle::jleGameEngine &ge) {
    ...
    if (ImGui::IsWindowHovered()) {
        ...
        jleEditor::mEditorCamera.ApplyPerspectiveMouseMovementDelta(glm::vec2{dragDelta.x, -dragDelta.y});

        if (ImGui::IsKeyDown(ImGuiKey_W)) {
            jleEditor::mEditorCamera.MovePerspectiveForward(115.f * ge.status->GetDeltaFrameTime());
        }
        if (ImGui::IsKeyDown(ImGuiKey_S)) {
            jleEditor::mEditorCamera.MovePerspectiveBackward(115.f * ge.status->GetDeltaFrameTime());
        }
        if (ImGui::IsKeyDown(ImGuiKey_D)) {
            jleEditor::mEditorCamera.MovePerspectiveRight(115.f * ge.status->GetDeltaFrameTime());
        }
        if (ImGui::IsKeyDown(ImGuiKey_A)) {
            jleEditor::mEditorCamera.MovePerspectiveLeft(115.f * ge.status->GetDeltaFrameTime());
        }
        if (ImGui::IsKeyDown(ImGuiKey_Space)) {
            jleEditor::mEditorCamera.MovePerspectiveUp(115.f * ge.status->GetDeltaFrameTime());
        }
        if (ImGui::IsKeyDown(ImGuiKey_LeftCtrl)) {
            jleEditor::mEditorCamera.MovePerspectiveDown(115.f * ge.status->GetDeltaFrameTime());
        }
