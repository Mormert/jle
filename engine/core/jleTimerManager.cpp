...
void jleTimerManager::executeFuncInSecondsSharedData(
    ...
    using namespace std::chrono;
    const auto futureTime = duration<double>(high_resolution_clock::now().time_since_epoch()).count();
...
void jleTimerManager::executeFuncInSecondsWeakData(
    ...
    using namespace std::chrono;
    const auto futureTime = duration<double>(high_resolution_clock::now().time_since_epoch()).count() + seconds;
