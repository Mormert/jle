...
public:
    ...
    create(int32_t allocateBytes = 10000000)
    {
        auto container = ComponentContainer{};
        container.data.reserve(allocateBytes);
        container.addComponentF = addComponentT<T>;
        container.allocateComponentsF = allocateComponentsT<T>;
        container.getComponentF = getComponentT<T>;
        container.removeComponentF = removeComponentT<T>;
        container.componentTypeName = getCleanTypeName(typeid(T).name());

        container.serializeInputF_JSON = serializeInputT_JSON<T>;
        container.serializeOutputF_JSON = serializeOutputT_JSON<T>;

        container.serializeInputF_Binary = serializeInputT_Binary<T>;
        container.serializeOutputF_Binary = serializeOutputT_Binary<T>;

#if JLECS_USE_DEBUG
        container.getComponentDebugF = getComponentDebugT<T>;
        auto debug = std::make_unique<ComponentContainerDebug<T>>(getCleanTypeName(typeid(T).name()), &container);
        debug->count = 0;
...
#if JLECS_USE_DEBUG
        thiz->debug->count = thiz->_componentCount;
#endif
...
private:
    std::vector<ComponentContainer> componentContainers;
