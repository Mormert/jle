...
struct TextureWithHeightmap{
    std::shared_ptr<jle::iTexture> texture{nullptr};
    std::shared_ptr<jle::iTexture> heightmap{nullptr};
    std::shared_ptr<jle::iTexture> normalmap{nullptr};
};

struct TexturedHeightQuad : public Quad {
    explicit TexturedHeightQuad(std::shared_ptr<TextureWithHeightmap> t) : mtextureWithHeightmap{std::move(t)} {}

    explicit TexturedHeightQuad(std::shared_ptr<TextureWithHeightmap> t, int texX, int texY, unsigned int w, unsigned int h)
            : mtextureWithHeightmap{std::move(t)}, textureX{texX}, textureY{texY}, width{w}, height{h} {}
...
public:
    ...
    virtual void SendTexturedHeightQuad(TexturedHeightQuad &texturedHeightQuad, RenderingMethod renderingMethod) = 0;
