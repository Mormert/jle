...
namespace jle {
    ...
    jleGameEngine::GetFramebufferDimensions(unsigned int windowWidth, unsigned int windowHeight) {
    ...
    void jleGameEngine::Start() {
        ...
        window->AddWindowResizeCallback(
                std::bind(&jleGameEngine::FramebufferResizeEvent, this, std::placeholders::_1, std::placeholders::_2));
    ...
    void jleGameEngine::Render() {
        if (!gameHalted && game) {
            ...
            fullscreen_renderer->RenderFramebufferFullscreen(*framebuffer_main.get(), window->GetWindowWidth(),
